    ...
    try:
        ...
        if not ticket_id or not requester_email:
            return api_response(
                400,
                {
                    "message": "Ticket ID and requester email are required."
                },
            )
        ...
        if not ticket:
            return api_response(
                ...
                {
                    "message": (
                        "Ticket not found or requester information "
                        "does not match."
                    )
                },
            )
        ...
        print(
            f"Customer ticket lookup failed: "
        ...
        return api_response(
            500,
            {
                "message": "Unable to look up ticket."
            },
        )